...
public class AuthorizationServerConfig {
    ...
    @Bean
    @Order(Ordered.HIGHEST_PRECEDENCE) // o do auhtorization server deve ser executado primeiro, para que o login funcione corretamente, seg tbm
    public SecurityFilterChain authFilterChain(HttpSecurity http) throws Exception { //bean que cuida do http security,
        // OAuth2AuthorizationServerConfiguration.applyDefaultSecurity(http);           aplicando os filtros de segurança
    ...
    @Bean
    public ProviderSettings providerSettings(AlgaFoodSecurityProperties properties) { // responsável por escrever quem seria o A-S que vai
        return ProviderSettings.builder()                                             // assinar os tokens
